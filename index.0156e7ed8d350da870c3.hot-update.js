...
const CarEdit = () => {
  ...
  (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    const allStates = [getFieldState('car'), getFieldState('category'), getFieldState('colorsArray')];
    const readyStates = allStates.filter(state => !state.invalid);
    console.log('Количество стейтов', allStates.length);
    console.log('Заполненные стейты', readyStates);
    const progressBarState = `${Math.floor(readyStates.length / allStates.length * 100)}%`;
    console.log('Прогресс', progressBarState);
  }, [currentCar, currentCategory, colorsCheckBoxes]);
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("210da53112d222a2aec9")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=index.0156e7ed8d350da870c3.hot-update.js.map